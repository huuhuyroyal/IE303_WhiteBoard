...
import {
  Plus, Search, LayoutGrid, MoreHorizontal, Clock, Pencil, Trash2, X, LogOut
} from 'lucide-react';
  ...
  if (board.thumbnail) {
    return (
      <div className="w-full h-full relative bg-slate-50">
        <img src={board.thumbnail} alt="Thumbnail" className="w-full h-full object-cover" />
      </div>
    );
  }
  ...
  return (
    ...
    >
      <div className="relative rounded-xl overflow-hidden aspect-[4/3] mb-3 ring-1 ring-slate-200 group-hover:ring-blue-400 group-hover:shadow-lg transition-all duration-200">
        ...
        {hovering && <div className="absolute inset-0 bg-black/10 transition-all" />}
        <button
      ...
      <div className="px-0.5">
        {renaming ? (
          <input
            ...
            onBlur={() => { onRename(board.id, newName); setRenaming(false); }}
            onKeyDown={e => {
              if (e.key === 'Enter') { onRename(board.id, newName); setRenaming(false); }
              if (e.key === 'Escape') setRenaming(false);
            }}
            onClick={e => e.stopPropagation()}
          />
        ) : (
          <p className="text-sm font-semibold text-slate-800 truncate">{board.name || 'Untitled'}</p>
        )}
    ...
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center">
      <div className="bg-white rounded-2xl shadow-2xl p-6 w-[360px]">
        ...
        <p className="text-sm text-slate-500 mb-6">
          <span className="font-semibold text-slate-700">"{boardName}"</span> will be permanently deleted. This action cannot be undone.
        </p>
        <div className="flex gap-3">
          <button onClick={onCancel} className="flex-1 py-2 rounded-lg border border-slate-200 text-sm text-slate-600 hover:bg-slate-50">
            Cancel
          </button>
          <button onClick={onConfirm} className="flex-1 py-2 rounded-lg bg-red-600 text-sm text-white font-semibold hover:bg-red-700">
            Delete
          </button>
        </div>
      </div>
...
export default function Dashboard() {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const recent = JSON.parse(localStorage.getItem(`recent_boards_${user?.username}`) || '{}');
        recent[created.id] = nowMs;
        localStorage.setItem(`recent_boards_${user?.username}`, JSON.stringify(recent));
  ...
  const handleDelete = async (id) => {
    try {
      await fetch(`${BASE}/api/board/${id}`, { method: 'DELETE', headers: authHeaders() });
    } catch {
  ...
  const recentHistory = JSON.parse(localStorage.getItem(`recent_boards_${user?.username}`) || '{}');
      ...
      <aside className="w-56 bg-white border-r border-slate-200 flex flex-col shrink-0">
        <div className="px-5 py-4 border-b border-slate-100">
          <div className="flex items-center gap-2">
            ...
            <span className="font-bold text-slate-800 text-base tracking-tight">Co-Canvas</span>
          </div>
        </div>
        ...
        <div className="px-4 py-3 border-b border-slate-100">
          <div className="flex items-center gap-2.5 p-2 rounded-lg">
            ...
            <span className="text-sm font-medium text-slate-700 flex-1 truncate">{user?.username || 'Guest'}</span>
          </div>
        </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-5">
              ...
              {filtered.map((board) => {
                ...
                return (
                  <BoardCard
                    ...
                    nowMs={nowMs}
                    onOpen={handleOpen}
                    onDelete={() => setDeleteTarget({ id: board.id, name: board.name })}
              ...
              {filtered.length === 0 && search && (
                <div className="col-span-full text-center py-16 text-slate-400">
                  ...
                  <p className="text-sm">No boards matching "<strong>{search}</strong>"</p>
                </div>
              )}
            </div>
          )}
